...
1 comprehensive
2. yes
3. include in this DISCUSS wave
...
i dont quite understand the "Rule Builder Field Autocomplete Strategy"

the context says:
"""
The rule builder needs to suggest valid field paths for policy predicates (e.g., action_spec.provider, budget_limit.amount). These fields come from the IntentEvaluationContext type. The question is whether the field list should be static (hardcoded) or dynamic (derived from runtime data).
"""

but policies can literally contain anything?  like a policy could be that any agent is not allowed to author...
