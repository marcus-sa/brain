...
status of task should also go from "in_progress" to "pending_human_review" or something like that

### Prompt 7

status of task should also go from "in_progress" to "failed" or something like that
...
depends on the failure. if its unrecoverable, such as a programming bug, then it should be reset
