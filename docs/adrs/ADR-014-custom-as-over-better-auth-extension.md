...
The Brain platform needs a DPoP-bound RAR token issuance layer. Two paths exist: extend the existing Better Auth OAuth provider plugin with DPoP+RAR support, or build a Custom Authorization Server as a new module within the Brain server.

Better Auth provides session management, OAuth 2.1 provider (authorization code + PKCE), JWKS, and JWT access tokens. However, it has no built-in support for DPoP proof validation, RAR authorization_details in tokens, or the custom grant type `urn:brain:intent-authorization`.

The Brain's authorization model requires tight integration with the intent evaluation pipeline (Policy Gate -> Authorizer Agent -> Risk Router) and the identity hub-spoke model. Token issuance must verify intent status, DPoP key binding, and brain_action matching -- none of which are standard OAuth flows.

## Decision

Build a Custom Authorization Server as a new `app/src/server/oauth/` module within the Brain server process. Better Auth remains the human identity provider (sessions, login, JWKS for session tokens). The Custom AS has its own signing key and JWKS endpoint for Brain access tokens.

The Custom AS:
- Issues DPoP-bound tokens with brain_action authorization_details
- Has its own ES256 signing key pair (separate from Better Auth)
- Exposes its own JWKS endpoint at `/api/auth/brain/.well-known/jwks`
...
- **Cons**: Better Auth's plugin API is designed for standard OAuth flows. DPoP proof validation, intent-status verification, and brain_action claim embedding require deep hooks into the token issuance pipeline that the plugin API may not expose. Tight coupling to Better Auth's internal token format and upgrade cycle. The `oauthProvider` plugin's `customAccessTokenClaims` callback cannot perform DPoP proof validation or intent verification.
...
Deploy the Custom AS as a separate Bun process communicating with the Brain server via HTTP.
...
- Clean separation: Better Auth = human identity, Custom AS = Brain authorization
...
- Two JWKS endpoints (Better Auth at `/api/auth/jwks`, Custom AS at `/api/auth/brain/.well-known/jwks`)
...
| Reliability | Same-process AS eliminates network partition risk between AS and Brain | Positive |
