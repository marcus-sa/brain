...
The Brain platform currently authenticates MCP/API requests via Bearer tokens issued by Better Auth's OAuth provider. The Sovereign Hybrid Model replaces this with DPoP-bound RAR tokens issued by the Custom Authorization Server.
...
3. Session cookies at the Brain boundary receive 401 "dpop_required"
...
- No classification boundary at the Brain. One auth path, one verification pipeline.
...
- All MCP clients (brain CLI plugin, agent runtimes) must update simultaneously
...
2. Update all MCP clients to use DPoP (brain CLI plugin, agent runtime libraries)
