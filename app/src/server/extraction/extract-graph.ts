  ...
  try {
    const extractionOutput = await generateObject({
      ...
      system: [
        ...
        "For declarative/spec sentences: classify enduring capability/process behavior as feature.",
        "For declarative/spec sentences: classify architecture/tool/provider selection as decision (for example store in SurrealDB, use Postgres, adopt OpenRouter).",
        "Classify execution directives with delivery/operator cues as task (for example implement, ship this week, fix by Friday).",
